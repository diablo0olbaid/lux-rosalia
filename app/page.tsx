...
import { GripVertical, Trash2, Download, Star, Image as ImgIcon } from "lucide-react";
  ...
  return (
    ...
    >
      ...
      <div className="absolute top-20 left-0 right-0 text-center">
        <h1
          className="text-[110px] font-light tracking-widest"
          style={{ color: palette.ink }}
        >
          LUX
        </h1>
        <p className="text-xl" style={{ color: palette.mist }}>
          Rosalía · 2025
        </p>
      </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {selection.map((name, i) => (
            <div key={i} className="flex items-center gap-4">
              <div
                className="w-12 h-12 rounded-full grid place-items-center text-xl font-semibold"
                style={{ background: palette.gold, color: "#151515" }}
              >
                {i + 1}
              </div>
              <div
                className="flex-1 p-4 rounded-2xl border backdrop-blur-sm"
                style={{
...
export default function LuxRankingApp() {
  ...
  const [accent] = useState<string>("Tu Top del Álbum");
  const [bgImage, setBgImage] = useState<string>("");

  const { onDragStart, onDragOver, onDrop } = useDragSort(tracks, setTracks);
  ...
  return (
    <div
      ...
      style={{
        background:
          `radial-gradient(1200px 800px at 20% -5%, #1b2f8a 0%, ${palette.bgDeep} 40%, #050816 100%)`,
      }}
    >
      <div className="max-w-7xl mx-auto px-4 md:px-8 py-10 md:py-14">
        <h1 className="text-3xl font-semibold mb-4" style={{ color: palette.ink }}>
          LUX – Fan Ranking
        </h1>
        <div className="flex items-center gap-3 mb-6">
          <label
            className="px-3 py-2 rounded-xl border cursor-pointer flex items-center gap-2"
          ...
          <button
            onClick={exportPNG}
            className="px-4 py-2 rounded-xl font-medium flex items-center gap-2 border"
        ...
        <ul className="space-y-2 mb-8">
          {tracks.map((t, i) => (
            <li
              ...
              onDrop={onDrop(i)}
              className="group flex items-center gap-3 px-3 py-2 rounded-xl border bg-white/5"
        ...
        <div className="flex items-center gap-4 mb-8">
          <label className="text-sm" style={{ color: palette.mist }}>
            Top N:
          </label>
          <input
            type="number"
            ...
            value={topN}
            onChange={(e) =>
              setTopN(clamp(parseInt(e.target.value || "1", 10), 1, tracks.length))
            }
            className="w-20 px-2 py-1 rounded bg-transparent border"
            style={{ borderColor: "rgba(255,255,255,0.2)", color: palette.ink }}
          />
        </div>
